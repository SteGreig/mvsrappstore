...
class Enhanced_Ecommerce_Google_Analytics_Admin extends TVC_Admin_Helper 
{
    ...
    public function enqueue_styles() {
      ...
      if ($screen->id === 'toplevel_page_conversios'  || (isset($_GET['page']) === TRUE && strpos(sanitize_text_field(filter_input(INPUT_GET,'page')), 'conversios') !== false) ) {
          ...
          if($screen->id !== CONV_SCREEN_ID.'conversios-google-shopping-feed'  && !isset($_GET['subpage']))
          {
            wp_enqueue_style('custom-css', esc_url_raw(ENHANCAD_PLUGIN_URL.'/admin/css/custom-style.css'), array(), esc_attr($this->version), 'all' );
          }
          wp_enqueue_style('conversios-header-css', esc_url_raw(ENHANCAD_PLUGIN_URL.'/admin/css/header.css'), array(), esc_attr($this->version), 'all' );
    ...
    public function enqueue_scripts() {
      ...
      if ($screen->id === 'toplevel_page_conversios'  || (isset($_GET['page']) === TRUE && strpos(sanitize_text_field(filter_input(INPUT_GET,'page')), 'conversios') !== false) ) {
          ...
          if ($this->is_current_tab_in(array('sync_product_page','gaa_config_page','gmcsettings')) 
          || $screen->id== CONV_SCREEN_ID."conversios-google-analytics" 
          || $screen->id== CONV_SCREEN_ID."conversios-google-shopping-feed") {
