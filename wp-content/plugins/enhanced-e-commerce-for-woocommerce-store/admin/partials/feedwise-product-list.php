    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-footer border-0 pb-4 mb-1">
                <a href="<?php echo esc_url_raw($site_url); ?>" type="button"
            ...
            <div class="col-9 pt-0">
                ...
                <span class="ms-2 ps-2 pe-1 pt-1 pb-1 rounded border">
                    <label class="fs-14 fw-500 text-info pb-1 defaultPointer">
                        ...
                        <label class="fs-14 fw-500 text-secondary">
                            <?php echo esc_html_e(number_format($result[0]['status'] === 'Synced' ? $result[0]['total_product'] : 0), "enhanced-e-commerce-for-woocommerce-store"); ?>
                        </label>
                    </label>
                </span>
            </div>
...
<div class="modal fade" id="filterModal" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel"
    ...
    <input type="hidden" name="syncProductCount" id="syncProductCount" value="<?php echo esc_html($google_detail_api->data->product_count) ?>">
...
<script>  
    ...
    var walk_through = "<?php echo $walk_through_product_list ?>";
    var walk_through_enable = "<?php echo $walk_through_product_list_enable ?>";
    function getData($type = "") { 
        var selected_vals = {};
        var walk_through = {};
        walk_through["product_list"] = $type;
        walk_through["product_list_enable"] = "no";
    ...
    function walk_tours(){
        ...
        {
                "next .filterDiv": 'You can apply filters from here to segregate the products as per your requirements.',
                "nextButton" : {className: "myNext", text: "Next"},
                "skipButton" : {className: "mySkip", text: "Skip"},
                onBeforeStart:function(){
                $("#categoryModal").scrollTop(0);
                jQuery('.enjoyhint_skip_btn').removeClass('skip_btn') 
                jQuery('.enjoyhint_prev_btn').removeClass('prev_btn')                                               
            }
            },                    
            {
                "click .filteredProductSyn": "Great let's move ahead to map your product categories and attributes.",
                "nextButton" : {className: "myNext", text: "Next"},
                "skipButton" : {className: "mySkip", text: "Skip"},
                onBeforeStart:function(){
                jQuery('.enjoyhint_skip_btn').addClass('skip_btn')
                jQuery('.enjoyhint_prev_btn').addClass('prev_btn')
    ...
    jQuery(document).ready(function () {        
        ...
        jQuery('.select2').select2({ dropdownParent: $("#filterModal") });
        ...
        var table = jQuery('#product_list_table').DataTable({
            ...
            if((feed_count == '1' && walk_through == '') || (walk_through == "" && walk_through_enable ==  "yes")){
                walk_tours();
            }
        ...
        jQuery(document).on('click', '#filterReset', function (event) {
            ...
            jQuery('.select2').select2({ dropdownParent: $("#allFilters") });
        });
        ...
        jQuery(document).on('click', '#filterSubmit', function (event) {
            //let count = $(".countCard").text();
            let product = $("select[name='product[]'] option:selected").map(function () { return $(this).val(); }).get();
            let producttext = $("select[name='product[]'] option:selected").map(function () { return $(this).text(); }).get();
            let condition = $("select[name='condition[]'] option:selected").map(function () { return $(this).val(); }).get();
            let value = $("input[name='value[]']").map(function () { return $(this).val(); }).get();
            let seltext = $("select[name='value[]'] option:selected").map(function () { return $(this).text(); }).get();
            let selVal = $("select[name='value[]'] option:selected").map(function () { return $(this).val() ? $(this).val() : ''; }).get();
            let flag = 0;
            let valFlag = 0;
            let prodData = Array();
        ...
        jQuery(document).on('click', '.filteredProductSyn', function (e) {            
            ...
            if (!$.isEmptyObject(productArray) || !$.isEmptyObject(exclude) || ($(this).val() == 'syncAll' && $('.syncProduct').is(':checked')) || !$.isEmptyObject(include)) {
                ...
                jQuery.ajax({
                    ...
                    success: function (response) {
                        ...
                        selected_cat_id.forEach(function(v,i){	
                            if(is_tvc_cat_selected == false && jQuery("#category-"+$.trim(v)).val() != "" && jQuery("#category-"+$.trim(v)).val() != 0){
    ...
    function conv_change_loadingbar(state = 'show') {
        if (state == 'show') {
            ...
            $("#wpwrap").css("pointer-events", "auto");
        }
    }
    ...
    function enable_tutorial(){
        ...
        var selected_vals = {};
        var walk_through = {};
        walk_through["product_list"] = "";
        walk_through["product_list_enable"] = "yes";
        walk_through["feed_list"] = "<?php echo $walk_through_feed_list ?>";
        walk_through["feed_list_enable"] = "<?php echo $walk_through_feed_list_enable ?>";
        walk_through["gmc_settings"] = "<?php echo $walk_through_gmc_settings ?>";
        walk_through["gmc_settings_enable"] = "<?php echo $walk_through_gmc_settings_enable ?>";
