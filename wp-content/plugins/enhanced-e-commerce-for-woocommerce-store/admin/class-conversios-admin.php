...
if (class_exists('Conversios_Admin') === FALSE) {
    class Conversios_Admin extends TVC_Admin_Helper
    {
      ...
      function dequeue_css()
      {
        ...
        global $wp_styles;
        if(str_contains($screen->id, CONV_SCREEN_ID) || $screen->id === 'toplevel_page_conversios') {      
      ...
      public function enqueue_styles()
      {
        ...
        if ($screen->id === 'toplevel_page_conversios'  || (isset($_GET['page']) === TRUE && strpos(sanitize_text_field($_GET['page']), 'conversios') !== false)) {
          ...
          if(isset($_GET['page']) && $_GET['page'] === "conversios-google-shopping-feed") {
            
            wp_enqueue_style('enjoyhint-css', esc_url_raw(ENHANCAD_PLUGIN_URL.'/admin/tour/enjoyhint.css'));
          }
        }
      }
      ...
      public function enqueue_scripts()
      {
        ...
        } else if (isset($_GET['page']) === TRUE && sanitize_text_field($_GET['page']) === "conversios-pmax") {
        ...
        if(isset($_GET['page']) && $_GET['page'] === "conversios-google-shopping-feed") {
          wp_enqueue_script('kinetic-min-js', esc_url_raw('https://cdnjs.cloudflare.com/ajax/libs/kineticjs/5.2.0/kinetic.js'));
          wp_enqueue_script('jquery-scrollTo-min-js', esc_url_raw('https://cdnjs.cloudflare.com/ajax/libs/jquery-scrollTo/2.1.2/jquery.scrollTo.min.js'));
          wp_enqueue_script('enjoyhint-min-js', esc_url_raw(ENHANCAD_PLUGIN_URL.'/admin/tour/enjoyhint.min.js'));
        }
      }
      ...
      // <!-- Enjoyhint library -->
      // <link href="enjoyhint.css" rel="stylesheet">
      ...
      public function add_admin_pages()
      {
        ...
        add_menu_page(
          esc_html__(CONV_TOP_MENU, 'enhanced-e-commerce-for-woocommerce-store'),
          esc_html__(CONV_TOP_MENU, 'enhanced-e-commerce-for-woocommerce-store') . '',
          'manage_options',
          CONV_MENU_SLUG,
          array($this, 'showPage'),
