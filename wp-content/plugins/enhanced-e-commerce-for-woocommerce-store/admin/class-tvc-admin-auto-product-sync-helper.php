...
if ( ! class_exists( 'TVC_Admin_Auto_Product_sync_Helper' ) ) {
  Class TVC_Admin_Auto_Product_sync_Helper{
    ...
    public function add_table_in_db(){       
      ...
      $query = $wpdb->prepare('SHOW TABLES LIKE %s', $wpdb->esc_like($tablename));
      if ($wpdb->get_var($query) === $tablename) {
        $query = $wpdb->prepare("SHOW COLUMNS FROM ".$tablename." LIKE %s", $wpdb->esc_like('is_default'));
        $result = $wpdb->get_var($query);
        if ($result == '') {
          $wpdb->query("ALTER TABLE $tablename ADD `is_default` int(11) NOT NULL DEFAULT '0' AFTER `is_delete`");
        }
        ...
        $query = $wpdb->prepare("SHOW COLUMNS FROM ".$tablename." LIKE %s", $wpdb->esc_like('is_super_feed'));
        $result = $wpdb->get_var($query);
        if ($result == '') {
          $wpdb->query("ALTER TABLE $tablename ADD `is_super_feed` int(11) NOT NULL DEFAULT '0'  AFTER `target_country`");
        } 
      ...
      $query = "SELECT count(*) as count from `$tablenamesync` where `feedId` is NULL";
          ...
          if(!empty($all_products)){
            foreach($all_products as $postkey => $postvalue){
              $t_data = array(
                ...
                'update_date'=>esc_sql(date('Y-m-d H:i:s'))
              );
