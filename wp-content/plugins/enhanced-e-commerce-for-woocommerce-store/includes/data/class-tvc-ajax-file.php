...
if (!class_exists('TVC_Ajax_File')):
  ...
  class TVC_Ajax_File extends TVC_Ajax_Calls
  {
          ...
          if ($TVC_Admin_DB_Helper->tvc_check_row("ee_product_feed", "id =" . $_POST['feedId'])) {
            ...
            if ($_POST['inculdeExtraProduct'] != '') {
              ...
              $feed_data = array(
                ...
                "status" => strpos($_POST['channel_ids'], '1') !== false ? esc_sql('In Progress') : null,
                "is_default" => esc_sql(0),
                "tiktok_status" => strpos($_POST['channel_ids'], '3') !== false ? esc_sql('In Progress') : null,
              );
              $TVC_Admin_DB_Helper->tvc_update_row("ee_product_feed", $feed_data, array("id" => $_POST['feedId']));
              ...
              $isSyncComplete = $TVCProductSyncHelper->manualProductSync($_POST['feedId']);
              if ($isSyncComplete['status'] === 'success') {
                ...
                if ($result[0]['auto_schedule'] == 1) {
                  ...
                  as_schedule_recurring_action(esc_attr($timestamp), esc_attr($time_space), 'init_feed_wise_product_sync_process_scheduler_ee', array("feedId" => $_POST['feedId']), "product_sync");
                }
    ...
    {
      if ($this->safe_ajax_call($_POST['create_superFeed_nonce'], 'create_superFeed_nonce_val')) {
        $TVC_Admin_Helper = new TVC_Admin_Helper();
        ...
        $feed_data_api = array(
          ...
          "map_categories" => json_encode($categories),
          "map_attributes" => json_encode($mappedAttrs),
          "filter" => '',
          "include" => '',
          "exclude" => '',
        ...
        if ($isSyncComplete['status'] == 'success') {
          ...
          echo json_encode(array('status' => 'success', 'message' => 'Product Sync Done!!!!', 'total_product' => $totalProduct));
        } else {
          $feed_data = array(
            "product_sync_alert" => $isSyncComplete['message'],
            "is_process_start" => false,
          ...
          $TVC_Admin_Helper->plugin_log("error", 'product_sync'); // Add logs
          $TVC_Admin_DB_Helper->tvc_update_row("ee_product_feed", $feed_data, array("id" => $result['id']));
          echo json_encode(array('status' => 'error', 'message' => 'Error in Product Sync'));
        }
