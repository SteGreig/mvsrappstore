...
function WooCommerce_MagicZoom_send_stat($action = '') {
    ...
    if('working' == 'v6.9.7' || 'working' == 'v5.3.7') {
        return;
    }
    ...
    $path = "api/stat/?action={$action}&tool_name=magiczoom&license=trial&tool_version=v5.3.7&module_version=v6.9.7&platform_name=woocommerce&platform_version={$platformVersion}&url={$url}";
...
function  magictoolbox_WooCommerce_MagicZoom_init() {
    ...
    delete_option("WooCommerceMagicZoomCoreSettings");
        add_option("WooCommerceMagicZoomCoreSettings", $allParams,'','no');
    }
...
function WooCommerce_MagicZoom_import() {
    ...
    if (array_key_exists('tool', $arr) && $tool == $arr['tool']) {
        ...
        if (array_key_exists('core', $arr)) {
            ...
            delete_option("WooCommerceMagicZoomCoreSettings");
            add_option("WooCommerceMagicZoomCoreSettings", $settings,'','no');
        }
...
function WooCommerce_MagicZoom_export() {
    ...
    WooCommerce_MagicZoom_wp_export(get_option("WooCommerceMagicZoomCoreSettings"), $secret_data, $value);
    exit;
}
...
function  magictoolbox_WooCommerce_MagicZoom_create($content) {
    ...
    if (isset($GLOBALS['MAGICTOOLBOX_'.strtoupper('magiczoom').'_SELECTORS']) && !empty($GLOBALS['MAGICTOOLBOX_'.strtoupper('magiczoom').'_SELECTORS']) && @count($GLOBALS['MAGICTOOLBOX_'.strtoupper('magiczoom').'_SELECTORS']) > 0) { //if there any additional images present
...
function WooCommerce_MagicZoom_get_product_variations ($product_id = false) {
    ...
    if ( $product->is_type('variable')) {
        ...
        if (is_array($variations) && count($variations) > 0) {
            ...
            $a = str_replace('<a','<a data-product-id="'.$product->get_id().'" data-magic-slide-id="zoom" onclick="onMagicZoomSelectorClick(this);" ',$a);
...
function WooCommerce_MagicZoom_get_containers_data($thumbs = array(), $post_id = false, $useWpImages = false) {
    ...
    if ((metadata_exists( 'post', $post_id, '_magic360_data' ) || metadata_exists( 'post', $main_id, '_magic360_data' )) && magictoolbox_WooCommerce_MagicZoom_check_plugin_active('magic360') ){
        ...
        if(!empty($magic360_plugin) && $magic360_plugin->params->checkValue('page-status','Yes') ){
            ...
            if(!empty($magic360_image_gallery)){
                ...
                if (!$useWpImages) {
                    $magic360_selector = '<a data-product-id="'.$post_id.'" data-magic-slide-id="360" style="display:inline-block;" class="m360-selector" title="360" href="#" onclick="return false;"><img src="'.WooCommerce_MagicZoom_get_product_image('/'.ltrim($magic360_selector_path,'/'),'selector').'" alt="360" /></a>';
                } else {
...
function WooCommerce_MagicZoom_slideshow_gallery($atts){
    ...
    $table_name = $wpdb->prefix . strtolower('MagicZoom_store');
